...
| `exclusions.yaml` | `exclusion_list_version` | Ethical denylist (§4.4 item 1), owner-reviewed 2026-09-13 (r1) |
...
| `fees.yaml` | part of `config_version` | Regulatory fee schedules, effective-dated (§9, ADR-0012; A-06) |
...
| `phase_change.yaml` | — | Sequence, reason and category the owner supplies when any of the above (or the worker code) changes (ADR-0019) |

Secrets never live here. They are Railway environment variables only (§11).

Prompts live in `prompts/<role>/vX.Y.Z.md` (see `prompts/README.md`); `prompt_version` is derived from them.
