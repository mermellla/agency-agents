...
class CorporateActionsAdapter(HealthCheckable, Protocol):
    async def actions(self, symbols: Sequence[str], start: date, end: date) -> list[dict[str, Any]]: ...
...
class RegimeClassifier(Protocol):
    async def classify(self, trade_date: date) -> tuple[MarketRegime, dict[str, Any]]: ...
...
class DossierBuilder(Protocol):
    async def slim(self, candidate: Candidate) -> dict[str, Any]: ...
    async def full(self, candidate: Candidate, portfolio_id: UUID) -> dict[str, Any]: ...


class LLMClient(Protocol):
    """§7.1 model tiers. Every call returns its LLMCall accounting row (§9)."""

    async def triage(self, dossiers: Sequence[dict[str, Any]], prompt_version: str) -> tuple[TriageResult, LLMCall]: ...
    async def decide(self, dossier: dict[str, Any], prompt_version: str) -> tuple[Proposal, LLMCall]: ...
    async def critique(self, proposal: Proposal, prompt_version: str) -> tuple[CritiqueResult, LLMCall]: ...
    async def finalize(
        self, proposal: Proposal, critique: CritiqueResult, prompt_version: str
    ) -> tuple[Proposal, LLMCall]: ...
    async def daily_review(self, position_dossier: dict[str, Any], prompt_version: str) -> tuple[Proposal, LLMCall]: ...
...
class RiskDesk(Protocol):
    ...
    async def validate(
        self, decision: Decision, virtual_cash: Decimal, open_positions: Sequence[Position]
    ) -> RejectCode | None: ...
...
class FillReconstructor(Protocol):
    ...
    async def reconstruct(
        self, request: OrderRequest, sim_fill_delay: timedelta, additional_slippage_bps: Decimal
    ) -> Fill | None: ...
...
class StopArmer(Protocol):
    ...
    async def rearm(self, session_date: date) -> list[dict[str, Any]]: ...
    async def verify_post_open(self, session_date: date) -> list[dict[str, Any]]: ...


class Reconciler(Protocol):
    """§8.6 replay-or-halt."""

    async def reconcile(self) -> tuple[ReconcileResult, dict[str, Any]]: ...
...
class BrokerPolicyEnforcer(Protocol):
    ...
    async def enforce(
        self, policy: BrokerPolicy, enforce_writes: bool
...
class Ledger(Protocol):
    ...
    async def transition_order(
        self, order_id: UUID, to_status: OrderStatus, reason: str, broker_event_id: str | None
    ) -> None: ...
...
class AnalysisJob(Protocol):
    async def report(self, experiment_id: UUID) -> dict[str, Any]: ...
