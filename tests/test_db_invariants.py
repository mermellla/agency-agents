...
@pytest.mark.parametrize(
    "table", ["decisions", "orders", "fills", "cash_ledger", "llm_calls", "closed_trades", "order_events"]
)
def test_no_physical_deletes(db, seed, table):
    ...
    db.execute(
        "insert into cash_ledger (portfolio_id, experiment_id, experiment_phase_id, kind, amount_usd, balance_after_usd, idempotency_key) values (%s, %s, %s, 'buy', -100, 400, %s)",
        (seed.primary_id, seed.experiment_id, seed.phase_id, uuid.uuid4().hex),
    )
    db.execute(
        "insert into llm_calls (experiment_id, experiment_phase_id, decision_id, stage, bucket, model, prompt_version, status) values (%s, %s, %s, 'decision', 'entries', 'm', %s, 'ok')",
        (seed.experiment_id, seed.phase_id, d, seed.versions["prompt_version"]),
    )
    if table == "closed_trades":
        pos = uuid.uuid4()
        db.execute(
            "insert into positions (id, portfolio_id, experiment_id, opened_in_phase_id, symbol, entry_decision_id, qty, status, closed_at) values (%s, %s, %s, %s, 'AAPL', %s, 0, 'closed', now())",
...
def test_order_eligible_equals_risk_validation(db, seed):
    with raises(db, "decisions_eligible_equals_validation"):
        seed.decision(
            risk_validation_completed_at=NOW + timedelta(minutes=4), order_eligible_at=NOW + timedelta(minutes=5)
        )
...
def test_short_must_be_rejected_ineligible(db, seed):
    ...
    seed.decision(
        decision="SHORT",
        status="rejected",
        reject_code="REJECTED_ACCOUNT_INELIGIBLE",
        forecast_target_price_at_entry=None,
        forecast_invalidation_price_at_entry=None,
        forecast_time_stop_at_entry=None,
    )
...
def test_legal_transitions_logged(db, seed):
    ...
    rows = db.execute(
        "select from_status, to_status, reason from order_events where order_id = %s order by id", (o,)
    ).fetchall()
...
def test_working_levels_can_move_but_resolution_uses_entry_levels(db, seed):
    ...
    seed.fill(o, fill_at=NOW + timedelta(minutes=20))
    db.execute(
        "update decisions set fill_at = %s, status = 'executed' where decision_id = %s",
        (NOW + timedelta(minutes=20), d),
    )
    pos = uuid.uuid4()
    db.execute(
        "insert into positions (id, portfolio_id, experiment_id, opened_in_phase_id, symbol, entry_decision_id, qty, working_target_price) values (%s, %s, %s, %s, 'AAPL', %s, 1, 110)",
        (pos, seed.primary_id, seed.experiment_id, seed.phase_id, d),
    )
    ...
    with raises(db, "FORECAST_RESOLVED_AGAINST_REVISED_LEVELS"):
        db.execute(
            """insert into forecast_resolutions (decision_id, position_id, resolution_start_at, target_at_entry, invalidation_at_entry, time_stop_at_entry, method, outcome, touched_at, touch_price)
                      values (%s, %s, %s, 130, 95, %s, 'bars_1m', 'TARGET', %s, 130)""",
            (d, pos, NOW + timedelta(minutes=20), NOW + timedelta(days=5), NOW + timedelta(hours=2)),
        )
    with raises(db, "FORECAST_RESOLUTION_START"):
        db.execute(
            """insert into forecast_resolutions (decision_id, position_id, resolution_start_at, target_at_entry, invalidation_at_entry, time_stop_at_entry, method, outcome, touched_at, touch_price)
                      values (%s, %s, %s, 110, 95, %s, 'bars_1m', 'TARGET', %s, 110)""",
            (d, pos, NOW, NOW + timedelta(days=5), NOW + timedelta(hours=2)),
        )
    db.execute(
        """insert into forecast_resolutions (decision_id, position_id, resolution_start_at, target_at_entry, invalidation_at_entry, time_stop_at_entry, method, outcome, touched_at, touch_price)
                  values (%s, %s, %s, 110, 95, %s, 'bars_1m', 'TARGET', %s, 110)""",
...
def test_ambiguous_needs_reason(db, seed):
    ...
    with raises(db, "check"):
        db.execute(
            """insert into forecast_resolutions (decision_id, resolution_start_at, target_at_entry, invalidation_at_entry, time_stop_at_entry, method, outcome)
                      values (%s, %s, 110, 95, %s, 'ticks', 'AMBIGUOUS')""",
            (d, NOW + timedelta(minutes=20), NOW + timedelta(days=5)),
        )
...
def test_cash_ledger_never_negative_and_chains(db, seed):
    args = (seed.primary_id, seed.experiment_id, seed.phase_id)  # seed already booked initial_equity 500
    with raises(db, "balance_after_usd|CASH_LEDGER_CHAIN_BROKEN"):
        db.execute(
            "insert into cash_ledger (portfolio_id, experiment_id, experiment_phase_id, kind, amount_usd, balance_after_usd, idempotency_key) values (%s, %s, %s, 'buy', -600, -100, 'k2')",
            args,
        )
    with raises(db, "CASH_LEDGER_CHAIN_BROKEN"):
        db.execute(
            "insert into cash_ledger (portfolio_id, experiment_id, experiment_phase_id, kind, amount_usd, balance_after_usd, idempotency_key) values (%s, %s, %s, 'buy', -100, 450, 'k3')",
            args,
        )
    db.execute(
        "insert into cash_ledger (portfolio_id, experiment_id, experiment_phase_id, kind, amount_usd, balance_after_usd, idempotency_key) values (%s, %s, %s, 'buy', -100, 400, 'k4')",
        args,
    )
    with raises(db, "UPDATE_FORBIDDEN"):
...
def test_cash_ledger_must_start_with_initial_equity(db, seed):
    fresh = uuid.uuid4()
    db.execute(
        "insert into portfolios (id, experiment_id, kind, name, touches_broker, equity_start_usd) values (%s, %s, 'llm_variant', 'v-fresh', false, 500)",
        (fresh, seed.experiment_id),
    )
    with raises(db, "CASH_LEDGER_MUST_START_WITH_INITIAL_EQUITY"):
        db.execute(
            "insert into cash_ledger (portfolio_id, experiment_id, experiment_phase_id, kind, amount_usd, balance_after_usd, idempotency_key) values (%s, %s, %s, 'buy', -100, 400, 'k1')",
            (fresh, seed.experiment_id, seed.phase_id),
        )
...
def test_one_broker_facing_portfolio(db, seed):
    with raises(db, "portfolios_one_broker_facing|portfolios_one_primary|check"):
        db.execute(
            "insert into portfolios (experiment_id, kind, name, touches_broker, equity_start_usd) values (%s, 'llm_variant', 'v2', true, 500)",
            (seed.experiment_id,),
        )
