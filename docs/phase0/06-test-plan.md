...
|---|---|---|---|---|---|
| T-01 | Virtual-cash cap: order above virtual cash rejected although the broker would accept it | DB ✔ + Integration + Paper | ✔ DB | test_reservation_required_and_bounded ✔ ($600 order on $500 refused at insert); test_virtual_cash_cap_ignores_broker_buying_power ✔ (desk context carries no broker buying power; reserved capital counts); paper run pending keys | S4 ✔ (paper pending) |
...
| T-03 | Horizon bounds and latency floor rejections | Unit | yes | test_horizon_bounds ✔, test_latency_floor_rejects_under_30min ✔ (incl. ADR-0011 15:00 ET intraday cut-off) | S4 ✔ |
...
| T-05 | Budget ledger: exhaust entry bucket → NO_ACTION for entries, exits still run, `BUDGET_OVERAGE_EXIT` logged | Integration | yes (mock LLM cost) | test_budget_entry_exhaustion_blocks_entries_not_exits ✔, test_allowance_prorates_and_carries_within_month ✔ | S4 ✔ |
| T-06 | Exclusion layer: seed denylist + SIC backstop; excluded name rejected at scanner prefilter **and** risk desk | Unit + Integration | ✔ screen + prefilter; desk pending | tests/test_exclusions.py ✔; tests/test_slice2_universe.py ✔; test_risk_desk_rejects_excluded_even_if_proposed ✔ (denylist and default-deny SIC at the desk; exits from excluded names allowed) | S2 ✔ / S4 ✔ |
...
| T-17 | Runaway guards: max orders/day, LLM calls/hour, consecutive rejects halt; every halt emails | Integration | yes | test_guards_halt_and_email ✔ (orders/day, consecutive rejects, LLM calls/hour with Jev exempt, kill switch off by default; halt row + notification row) | S4 ✔ |
...
| T-32 | QB-1.0: rules, `order_eligible_at` = scan completion + validation, no drift judgment | Integration | yes | test_qb_rules_v1_and_eligibility_earlier_than_llm ✔, test_qb_exit_on_stop_then_closed_trade_with_fees_and_benchmarks ✔ (deterministic exits, closed_trades two P&L views) | S4 ✔ |
| T-33 | Fees and dividends applied in every mode; two P&L views | Unit | yes | test_fee_engine_schedule_on_reconstructed_sell ✔, test_dividend_credit ✔; fees applied by apply_fill in every mode (S3) | S4 ✔ |
...
| T-36 | Daily digest email content and notification audit row | Unit | yes | test_daily_digest ✔ (Jev-tagged subject, rules fallback, notifications row), test_resend_sender_posts_and_failures_are_recorded ✔ | S4 ✔ |
| T-37 | Approval-link flow with expiry (LIVE only) | Integration | yes | `test_approval_link_expiry` | S9 (gated) |
| T-38 | Benchmarks: SPY/VTI bought at start close, marked daily; cash flat | Unit | yes | covered in test_qb_exit_on_stop_then_closed_trade_with_fees_and_benchmarks ✔ (SPY/VTI bought at the start close, marked daily, cash flat, closed-trade benchmark windows) | S4 ✔ |
